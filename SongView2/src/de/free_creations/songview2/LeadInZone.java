 ...
 */
class LeadInZone extends Zone {

  LeadInZone(SongCanvas canvas) {
    super(canvas);
  ...
  protected Paint makePaint() {
    ...
    return new GradientPaint(getLeftBorder(), 0, getColor(),
            getRightBorder() + 30, 0, canvas.getBackground());
  }

  @Override
  protected boolean isSweetspot(int x, int y) {
    int sw_left = getRightBorder();
  ...
  public int getRightBorder() {
    return canvas.getDimensions().getLeadInEndPixel();
  }
  ...
  protected void processRightBorderChanges(Object eventTag, Object oldValue, Object newValue) {
    if (eventTag == Prop.LEADINEND_PIXEL) {
  ...
  public void setDraggedBorder(int newValue) {
    canvas.getDimensions().setLeadInEndPixel(newValue);
  }
}
